...
class Game(object):
    ...
    def __init__(self, board, **kwargs):
        ...
        self.max_actions = int(kwargs.get('max_actions', 10000))
    ...
    def start(self):
        ...
        ai = MCTS
        ai.interface_init(self.board, [p1, p2], self.n_in_row, self.time, self.max_actions)
        ...
        while(1):
            ...
            move = player_in_turn.interface_get_action()
            self.board.update(p, move)
            self.graphic(self.board, human, ai)
            end, winner = self.game_end(ai, move)
            if end:
                if winner != -1:
    ...
    def game_end(self, ai, move):
        win, winner = ai.interface_has_a_winner(move)
        if win:
            return True, winner
...
def run():
    n = 3
    try:
        board = Board(width=4, height=4, n_in_row=n)
